...
$('connect').onclick=()=>{const name=$('name').value.trim(),room=$('room').value.trim();if(!name||!room)return;myId=crypto.randomUUID();socket=new WebSocket(`${location.protocol==='https:'?'wss':'ws'}://${location.host}/ws`);socket.onopen=()=>{$('status').textContent='CONNECTED';$('join').hidden=true;$('game').hidden=false;socket.send(JSON.stringify({type:'join_room',player_id:myId,name,room_id:room}))};socket.onclose=()=>{$('status').textContent='OFFLINE';$('join').hidden=false;$('game').hidden=true};socket.onmessage=e=>{const m=JSON.parse(e.data);if(m.state)render(m.state);switch(m.type){case'chat':add(`<b>[${esc(m.name)}]</b> ${esc(m.message)}`);break;case'welcome':add(`> ${esc(m.message)}`);break;case'player_joined':add(`> ${esc(m.name)} joined the channel`);break;case'player_left':add(`> ${esc(m.name)} left the channel`);break;case'node_result':add(`<b>> ${esc(m.node)}</b> ${esc(m.message)}`);break;case'puzzle_result':closePuzzle();add(`<b>> ${esc(m.node)}</b> ${esc(m.message)}`);break;case'puzzle_error':$('puzzleError').textContent=m.message;$('puzzleError').hidden=false;add(`> ${esc(m.node)}: ${esc(m.message)}`);break;case'game_started':add('> SYSTEM LOCKDOWN STARTED — MOVE');break;case'game_won':showResult('ACCESS GRANTED','ESCAPE SUCCESSFUL');add('> CONNECTION TERMINATED — WELL DONE, OPERATIVES');break;case'game_lost':showResult('SYSTEM LOCKDOWN','TIME EXPIRED');add('> NETWORK SILENT');break;case'error':add(`> ERROR: ${esc(m.message)}`)}}};
